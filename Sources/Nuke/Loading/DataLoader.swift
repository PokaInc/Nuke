...
public final class DataLoader: DataLoading, @unchecked Sendable {
    ...
    public var delegate: (any URLSessionDelegate)? {
    ...
    public init(configuration: URLSessionConfiguration = DataLoader.defaultConfiguration,
                validate: @Sendable @escaping (URLResponse) -> (any Swift.Error)? = DataLoader.validate) {
    ...
    @Sendable public static func validate(response: URLResponse) -> (any Swift.Error)? {
    ...
    public func loadData(with request: URLRequest,
                         ...
                         completion: @escaping ((any Swift.Error)?) -> Void) -> any Cancellable {
...
private final class _DataLoader: NSObject, URLSessionDataDelegate, @unchecked Sendable {
    let validate: @Sendable (URLResponse) -> (any Swift.Error)?
    private var handlers = [URLSessionTask: _Handler]()
    var delegate: (any URLSessionDelegate)?

    init(validate: @Sendable @escaping (URLResponse) -> (any Swift.Error)?) {
    ...
    func loadData(with task: URLSessionDataTask,
                  session: URLSession,
                  didReceiveData: @escaping (Data, URLResponse) -> Void,
                  completion: @escaping ((any Error)?) -> Void) -> any Cancellable {
    ...
    func urlSession(_ session: URLSession, didCreateTask task: URLSessionTask) {
        if #available(iOS 16.0, macOS 13.0, tvOS 16.0, watchOS 9.0, *) {
            (delegate as? any URLSessionTaskDelegate)?.urlSession?(session, didCreateTask: task)
        } else {
    ...
    func urlSession(_ session: URLSession,
        ...
        (delegate as? any URLSessionDataDelegate)?.urlSession?(session, dataTask: dataTask, didReceive: response, completionHandler: { _ in })
    ...
    func urlSession(_ session: URLSession, task: URLSessionTask, didCompleteWithError error: (any Error)?) {
        (delegate as? any URLSessionTaskDelegate)?.urlSession?(session, task: task, didCompleteWithError: error)
    ...
    func urlSession(_ session: URLSession, task: URLSessionTask, didFinishCollecting metrics: URLSessionTaskMetrics) {
        (delegate as? any URLSessionTaskDelegate)?.urlSession?(session, task: task, didFinishCollecting: metrics)
    }
    ...
    func urlSession(_ session: URLSession, task: URLSessionTask, willPerformHTTPRedirection response: HTTPURLResponse, newRequest request: URLRequest, completionHandler: @Sendable @escaping (URLRequest?) -> Void) {
        (delegate as? any URLSessionTaskDelegate)?.urlSession?(session, task: task, willPerformHTTPRedirection: response, newRequest: request, completionHandler: completionHandler) ?? completionHandler(request)
    }

    func urlSession(_ session: URLSession, taskIsWaitingForConnectivity task: URLSessionTask) {
        (delegate as? any URLSessionTaskDelegate)?.urlSession?(session, taskIsWaitingForConnectivity: task)
    }
    ...
    func urlSession(_ session: URLSession, task: URLSessionTask, didReceive challenge: URLAuthenticationChallenge, completionHandler: @Sendable @escaping (URLSession.AuthChallengeDisposition, URLCredential?) -> Void) {
        (delegate as? any URLSessionTaskDelegate)?.urlSession?(session, task: task, didReceive: challenge, completionHandler: completionHandler) ??
    ...
    func urlSession(_ session: URLSession, task: URLSessionTask, willBeginDelayedRequest request: URLRequest, completionHandler: @Sendable @escaping (URLSession.DelayedRequestDisposition, URLRequest?) -> Void) {
        (delegate as? any URLSessionTaskDelegate)?.urlSession?(session, task: task, willBeginDelayedRequest: request, completionHandler: completionHandler) ??
    ...
    func urlSession(_ session: URLSession, dataTask: URLSessionDataTask, didReceive data: Data) {
        (delegate as? any URLSessionDataDelegate)?.urlSession?(session, dataTask: dataTask, didReceive: data)
    ...
    func urlSession(_ session: URLSession, dataTask: URLSessionDataTask, willCacheResponse proposedResponse: CachedURLResponse, completionHandler: @Sendable @escaping (CachedURLResponse?) -> Void) {
        (delegate as? any URLSessionDataDelegate)?.urlSession?(session, dataTask: dataTask, willCacheResponse: proposedResponse, completionHandler: completionHandler) ??
    ...
    private final class _Handler: @unchecked Sendable {
        ...
        init(didReceiveData: @escaping (Data, URLResponse) -> Void, completion: @escaping ((any Error)?) -> Void) {
